...
model_options <- within(list(), {
  ...
  InstallDir<- simplace::findFirstSimplaceInstallation() # Simplace java classes
  WorkDir <- paste0(getwd(),'/simulation/') # solution and data files for the simulations
  OutputDir <- paste0(getwd(),'/output/') # Simplace output directory
  
  SolutionFile <-paste0(WorkDir,"solution/YieldSlaLueRgrl.sol.xml") # Solution to run

  # Define situations - e. g. load them from a project data file
  ProjectDataFile <- paste0(WorkDir, "data/climatezones.csv")
  ProjectData <- if(is.null(ProjectDataFile)){NULL}else{readr::read_delim(ProjectDataFile, delim=";")}
...
param_info <- list(lb=c(vLUE=0,vSLA=0,vRGRL=0.00001),ub=c(vLUE=10,vSLA=0.1,vRGRL=0.05))

#' Set the options for CroptimizeR
optim_options=list(path_results = paste0(getwd(),'/output'),   # path where to store the results (graph and Rdata)
