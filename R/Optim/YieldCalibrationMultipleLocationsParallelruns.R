...
#' subtitle: Calibration for different locations - setup for parallel runs.
...
#'
#' In this setup, the weather data is in one file, so that the simulations for
#' all locations share the same project id and can be therefore run in parallel
#' by Simplace.
...
solution <- paste0(workdir, "solution/YieldSlaLueRgrlAll.sol.xml") # Solution
zonedata <- read.delim(paste0(workdir, "data/climatezones.csv"),
                  sep = ";", header = TRUE) # climatezone IDs of locations
...
yield <- function(p, zones) {
  ...
  for (zone in zones) {
    ...
    createSimulation(simplace, parameter, queue = TRUE)
  }
  runSimulations(simplace)
  result <- getResult(simplace, "Yields")
